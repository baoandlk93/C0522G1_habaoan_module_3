...
-- join 3 bảng lấy thêm thông tin tên loại lớp
select s.id,s.`name` as student_name,c.`name` as class_name, ct.name as class_type_name
from student s
inner join class c
on s.class_id = c.id
join class_type ct
...
-- 5. Lấy ra các học viên có điểm lớn hơn 5 .
select *
from student s 
where s.point >5;
...
select s.point, count(s.point) as sl
from student s 
group by s.point;
